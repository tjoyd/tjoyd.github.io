...
function get11(){
  const newtarget = document.getElementById("tTeam");
  function handleIntersection(entries) {
    entries.map((entry) => {
      if (entry.isIntersecting && t3 == 0) {
        i=0
        ...
        function typeWriter(){
          if (i < team.length) {
            document.getElementById("tteam").innerHTML = document.getElementById("tteam").innerHTML + team.charAt(i);
            i++;
            setTimeout(typeWriter, speed);
          }
        }
        typeWriter();
        setTimeout(() => {
          document.getElementsByClassName("teamcont")[0].style.opacity = "1"
        }, speed*team.length);
        setTimeout(() => {
